...
const Pokedex = ({allPokemon}) => {
  ...
  const StatCard = ({pokemon}) => (
    <article key={pokemon.id}>
      ...
      <StatsList>
        <Stat><StatTitle>HP:</StatTitle> {pokemon.hp}</Stat>
        <Stat><StatTitle>Types:</StatTitle> {pokemon.types.join(', ')}</Stat>
        <Stat><StatTitle>Moves:</StatTitle> {pokemon.moves && pokemon.moves.join(', ')}</Stat>
      </StatsList>
    </article>
  )
    ...
    <main>
      <section style={{display: 'flex', width: '100%', padding: '50px'}}>
        <aside style={{display: 'block', minWidth: '300px', paddingRight: '20px', lineHeight: '1.4'}}>
          <div style={{position: 'sticky', width: '280px', maxHeight: 'calc(100vh - 140px)', overflow: 'scroll', top: "0px"}}>
